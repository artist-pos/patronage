...
export const metadata: Metadata = {
  title: "Collection — Studio — Patronage",
};

export default async function StudioCollectionPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/auth/login?redirect=/studio/collection");
      ...
      <div className="space-y-12">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="space-y-1">
            <h2 className="text-xl font-semibold tracking-tight">Collection</h2>
