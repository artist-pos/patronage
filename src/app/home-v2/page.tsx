...
// Hidden preview of a value-proposition-led homepage. Same data and components
// as `/`; excluded from search and the sitemap. `?hero=c` (default) merges the two:
// opportunity first, then the profile. `?hero=a` is opportunity-only, `?hero=b`
// is profile-first.
...
// Hero photos. Each carries its own theme, because what works on a near-black
// photo (white stats, dark-tinted glass text) disappears on a light one.
//   ?photo=blur (default) — soft cream/grey macro, light
//   ?photo=iris           — dried iris on black, dark
const PHOTOS = {
  blur: {
    src: "/images/hero-blur.webp",
    width: 1600,
    height: 1600,
    // Light photo: glass reads as a paler frosted pane, edged by a soft shadow.
    base: "bg-[#E9E6DF]",
    glass:
      "border border-white/60 bg-white/55 shadow-[0_8px_40px_rgba(0,0,0,0.14)] backdrop-blur-2xl backdrop-saturate-150",
    vars: undefined as React.CSSProperties | undefined,
  ...
  iris: {
    src: "/images/hero-iris.webp",
    width: 1600,
    height: 900,
    // Dark photo: translucent white reads as grey, so the greys inside the
    // panels are darkened (scoped custom properties) to keep small text and the
    // red/amber countdowns legible.
    base: "bg-black",
    glass:
      "border border-white/40 bg-white/55 shadow-[0_8px_40px_rgba(0,0,0,0.35)] backdrop-blur-2xl backdrop-saturate-150",
    vars: {
      "--fg-muted": "#1f1f1f",
      "--fg-subtle": "#262626",
      "--urgent": "#700F0F",
    ...
    statN: "text-white",
    statL: "text-white/75",
    credit: "text-white/70 hover:text-white",
    label: "text-[#5E140D]",
  },
} as const;
...
const HEROES = {
  c: {
    lead: "Find an opportunity.",
    // The H1 rolls through what each audience comes here to find.
  ...
  a: {
    lead: "Find an opportunity.",
    rotate: [],
    muted: "Send one link with every application.",
    body: "Grants, residencies, commissions and open calls for artists in Aotearoa and beyond. Your profile holds your bio, CV and work, ready to send with any application. {url}",
    profileLead: "",
  },
  b: {
    lead: "Build your profile.",
    rotate: [],
    muted: "Find what’s next.",
    body: "One place for your CV, portfolio and practice, matched to the grants, residencies, commissions and open calls open right now.",
    profileLead: "",
  },
} as const;
...
interface Props {
  searchParams: Promise<{ hero?: string; photo?: string }>;
}

export default async function HomeV2({ searchParams }: Props) {
  const { hero: heroParam, photo: photoParam } = await searchParams;
  const photo = PHOTOS[photoParam === "iris" ? "iris" : "blur"];
  const hero = HEROES[heroParam === "a" ? "a" : heroParam === "b" ? "b" : "c"];
  ...
  return (
    <div>
      {/* Smooth in-page scrolling for "Find out more", and a lighter (75%) frosted
          header so the hero photo reads through it. Page-scoped for the preview;
          moves to globals/Header on cutover. */}
      ...
      <section className={`relative isolate -mt-[53px] overflow-hidden lg:flex lg:min-h-[100svh] lg:flex-col lg:justify-center ${photo.base}`}>
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className={`max-w-[580px] p-6 sm:p-9 ${photo.glass}`} style={photo.vars}>
            ...
            <h1 className="t-display max-w-[640px] text-[40px] sm:text-[52px]">
              {hero.rotate.length > 0 ? <RollingHeadline prefix="Find" phrases={[...hero.rotate]} /> : hero.lead}
              {hero.muted && (
                <>
                  <br />
                  <span className={photo.muted}>{hero.muted}</span>
                </>
              )}
            </h1>
            {bodyParts.length > 1 ? (
              ...
              <p className="t-body mt-6 max-w-[520px] text-[color:var(--fg-muted)]">{hero.body}</p>
            )}
        ...
        <div className={`self-start p-5 sm:p-6 ${photo.glass}`} style={photo.vars}>
